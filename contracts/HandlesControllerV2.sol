 ...
 */
contract HandlesControllerV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct HandlesControllerStorage {
        ...
        mapping(address => uint256) greeting;

    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.HandlesController")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant HandlesControllerStorageLocation = 0x9b779b17422d0df92223018b32b4d1fa46e071723d6817e2486d003becc55f00;
    ...
    function greeting(address _user) external view returns (uint256) {
        HandlesControllerStorage storage $ = _getHandlesControllerStorage();
        return $.greeting[_user];
    }
    ...
    function greet() external onlyOwner {
        HandlesControllerStorage storage $ = _getHandlesControllerStorage();
        uint256 count = $.greeting[msg.sender];
        $.greeting[msg.sender] = count + 1;
